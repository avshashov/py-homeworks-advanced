...
def search_vacancies(page_source):
    bs = BeautifulSoup(page_source, 'lxml')
    vacancies = bs.findAll('a', class_='serp-item__title')
    return vacancies


def check_key_words(words, bs):
...
def vacancy_parser(vacancies):
    ...
    result = []
    for vacancy in tqdm(vacancies, desc='processing'):
        ...
        if check_key_words(['Django', 'Flask'], bs):
...
if __name__ == '__main__':
    ...
    try:
        ...
        vacancies = search_vacancies(page_source)
